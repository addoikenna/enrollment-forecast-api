...
# ---------------------------------------------------------
def backfill_all_programs_forecast(month="2026-06-01"):
    snapshot_date = pd.Timestamp.today().strftime("%Y-%m-%d")
    target_month = pd.Timestamp(month)

    history_df = load_forecast_history()

    existing_all_programs = history_df[
        (
            history_df["forecast_month"].dt.to_period("M")
            == target_month.to_period("M")
        )
    ...
    if len(existing_all_programs) > 0:
        return {
            "status": "skipped",
            "message": f"All Programs forecast already exists for {month}",
            "rows_sent": 0
        }
    ...
    if len(target_row) == 0:
        raise ValueError(
            f"No model forecast found for {month}"
        )
    ...
    for _, row in daily_forecast.iterrows():
        rows.append({
            ...
            "month_type": row["month_type"],
            "monthly_forecast": monthly_forecast,
            "model_version": MODEL_VERSION + "_repair"
        })
